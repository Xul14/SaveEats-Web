...
export function ModalFreteAreaEntrega({ isOpenModal, setModalOpenModalAreaEntrega, onCreateAreaEntrega, onUpdateAreaEntrega, AreaEntregaEmEdicao }) {
    ...
    //         try {
    //             const response = await axios.get(`https://save-eats-backend.azurewebsites.net/v1/saveeats/restaurante/raio-entrega/idRestaurante/${idRestaurante}`)
    ...
    useEffect(() => {
        async function areaEntregaData() {
            try {
                const areaEntregaResponse = await axios.get(`https://save-eats-backend.azurewebsites.net/v1/saveeats/restaurante/frete-area-entrega/idRestaurante/${idRestaurante}`);
        ...
        if (isEditing) {
            try {
                const response = await axios.put(
                    `https://save-eats-backend.azurewebsites.net/v1/saveeats/frete/area/entrega/id/${idAreaEntrega}`,
                    novaAreaEntrega,
                    {
                        headers: {
                ...
                const response = await axios.put(`https://save-eats-backend.azurewebsites.net/v1/saveeats/frete/area/entrega/id/${idAreaEntrega}`)
                console.log(response);
                console.error("Erro ao editar:", error);
            }
        } else {
            try {
                const response = await axios.post("https://save-eats-backend.azurewebsites.net/v1/saveeats/restaurante/frete-area-entrega", novaAreaEntrega)
